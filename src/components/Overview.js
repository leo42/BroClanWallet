...
function Overview(props) {
  ...
  const addressSettings = (address) =>
    <div className="addressSettings">
      { wallet.getDefaultAddress() === address ? "" : <button onClick={() => props.root.setDefaultAddress(address)}> Make Default</button> }
      <br/>
      <label>
        <span>Name</span>
  ...
  return (
    ...
    <div>
      Overview:
      <svg className="settingsIcon" id="meteor-icon-kit__solid-copy-s" fill="none" onClick={() =>setWalletSettingsOpen(!walletSettingsOpen)}>
...
{ walletSettingsOpen ?  settingsMenu(showingAddress) : "" }
      ...
      {Object.keys(wallet.getBalanceFull(showingAddress)).map((asset, index) => (
          <div className='overviewTokenContainer' key={index}><TokenElement tokenId={asset} amount={wallet.getBalanceFull(showingAddress)[asset]} filter={showing} search={search} /></div>
        ))}
